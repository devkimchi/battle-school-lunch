...
학생/학부모/교직원 등 누구나 학교 이름의 일부만으로 학교를 검색하고, 선택한 학교의 **중식(점심)** 식단을 원하는 날짜 범위에서 한눈에 확인할 수 있도록 한다. 상단 탭에서 급식 분석 채팅 UI로 전환해 향후 분석 기능에 사용할 질문을 작성하고 로컬 대화 목록에서 확인할 수 있다.
...
| G8 | 급식 분석 화면에서 여러 줄 질문을 작성하고 로컬 대화 목록에 추가할 수 있다. |
...
- 모바일 네이티브 앱
- 분석 질문에 대한 AI 응답 및 분석 API 연동
- 분석 대화의 서버 전송, 영속 저장 및 화면 이탈 후 복원
...
1. 초기 화면에 분석 질문 작성을 안내하는 빈 상태를 표시한다.
2. 사용자가 입력한 공백이 아닌 질문을 사용자 메시지로 로컬 목록에 추가하고 입력창을 비운다.
3. 일반 `Enter`는 입력창 안에서 줄바꿈한다.
4. `Ctrl+Enter` 또는 macOS의 `Command+Enter`는 전송 버튼과 동일하게 폼을 제출한다.
5. 현재 범위에서는 메시지를 서버로 전송하거나 분석 응답을 생성하지 않는다.
...
- **FR-A4** 분석 입력이 공백뿐이면 전송할 수 없다.
- **FR-A5** 분석 메시지는 현재 페이지의 프론트엔드 상태에만 추가하며 API를 호출하지 않는다.
- **FR-A6** 분석 입력창에서 `Enter`는 줄바꿈하고 `Ctrl+Enter` 또는 `Command+Enter`는 폼을 제출한다.
...
- [x] 분석 탭에서 빈 상태와 입력 폼을 표시하고 공백이 아닌 질문을 로컬 메시지 목록에 추가한다.
- [x] 분석 입력창에서 `Enter`는 줄바꿈하고 `Ctrl+Enter`와 `Command+Enter`는 질문을 전송한다.
- [x] 분석 채팅은 백엔드 API를 호출하거나 자동 응답을 생성하지 않는다.
...
| 8 | 분석 채팅의 이번 구현 범위는? | **프론트엔드 채팅 UI만 구현**하고 서버 전송·자동 응답은 제외 |
| 9 | 분석 질문 입력의 Enter 동작은? | `Enter`는 줄바꿈, `Ctrl+Enter`/`Command+Enter`는 전송 |
